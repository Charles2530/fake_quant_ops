    ...
    if target_values is not None:
        ...
        if block_size > 0:
            ...
            if len(A_shape) >= 2 and A_shape[-1] == block_size:
                ...
                max_blocks = min(1000, num_blocks)
                if num_blocks > max_blocks:
                    block_indices = np.random.choice(num_blocks, size=max_blocks, replace=False)
        ...
        if len(A_before_filtered) > max_samples:
            indices = np.random.choice(len(A_before_filtered), size=max_samples, replace=False)
...
def analyze_folder_value_distribution(folder_path, elem_format='fp4_e2m1', 
    ...
    # Limit the number of blocks to plot
    ...
    if len(plot_blocks) > 0:
        try:
            ...
            for block_idx, (block_data, shared_exp_val) in enumerate(zip(plot_blocks, plot_shared_exp)):
                # Create figure with two subplots for this block
                fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14.0, 2.5))
                ...
                # Set labels and styling for left subplot
                ax1.set_xlabel('Position in Block', fontsize=9, fontweight='normal')
                ax1.set_ylabel('Original Value', fontsize=9, fontweight='normal')
                ax1.tick_params(axis='both', which='major', labelsize=8)
                ax1.tick_params(axis='both', which='minor', labelsize=7)
                ax1.grid(False)
                ax1.set_title('Max-Scaling Scheme', fontsize=9, fontweight='normal')
                ...
                green_representable_s2 = [v * scale_factor_s2 for v in green_values]
                gray_representable_s2 = [v * scale_factor_s2 for v in gray_values]
                
                # Mark gray values
                for val in gray_values:
                    representable_val = val * scale_factor_s2
                ...
                # Set labels and styling for right subplot
                ax2.set_xlabel('Position in Block', fontsize=9, fontweight='normal')
                ax2.set_ylabel('Original Value', fontsize=9, fontweight='normal')
                ax2.tick_params(axis='both', which='major', labelsize=8)
                ax2.tick_params(axis='both', which='minor', labelsize=7)
                ax2.grid(False)
                ax2.set_title('Half-Scaling Scheme', fontsize=9, fontweight='normal')
                ...
                red_gray_mse = float(np.sum((block_array - red_gray_quantized) ** 2))
                ...
                if np.any(round_mask):
                    ...
                    round_mse = float(np.sum(round_errors ** 2))/32
                ...
                # Replace one random point with a new random value
                ax1.scatter(x_positions, block_array, s=2, alpha=0.3, c='#1f77b4', zorder=2)
                ax2.scatter(x_positions, block_array, s=2, alpha=0.3, c='#1f77b4', zorder=2)

                if red_gray_mse < gray_green_mse:
                    print(f"Block {block_idx+1} skipped because red_gray_mse < gray_green_mse")
                    plt.close()
                    continue
                
                # Add MSE text annotations on plots
                # Order: clip_mse, round_mse, avg_mse
                # Left subplot (Gray + Red): clip_mse=0, round_mse, avg_mse (red_gray_mse)
                ax1.text(0.02, 0.98, f'clip_mse = 0\nround_mse = {red_gray_mse:.6f}\navg_mse = {avg_mse_red_gray:.6f}', 
                ...
                # Right subplot (Gray + Green): clip_mse, round_mse, avg_mse (gray_green_mse)
                ax2.text(0.02, 0.98, f'clip_mse = {clip_mse:.6f}\nround_mse = {round_mse:.6f}\navg_mse = {avg_mse_gray_green:.6f}', 
                        transform=ax2.transAxes, fontsize=7, verticalalignment='top',
                        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='gray', linewidth=0.5))
                
                # Improve layout for two subplots
                plt.tight_layout(pad=1.0)
                
                # Background for figure
                fig.patch.set_facecolor('white')
                
                # Save plot for this block
                block_plot_path = output_dir / f'per_block_{block_idx+1:04d}_{elem_format}_{plot_name}.pdf'
                plt.savefig(block_plot_path, dpi=600, bbox_inches='tight', pad_inches=0.05, 
                ...
                mse_results.append({
                    ...
                    'pdf_file': str(block_plot_path.name),
